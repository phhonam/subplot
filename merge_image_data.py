...
def merge_image_data():
    ...
    for title, profile in profiles_data.items():
        ...
        if image_info:
            ...
            # Only merge director if it's not already present and the image_info director is valid
            if not profile.get('director') and image_info['director'] and image_info['director'].strip():
                # Additional validation: check if the director name looks reasonable
                director_name = image_info['director'].strip()
                if len(director_name) > 2 and not director_name.lower() in ['unknown', 'n/a', 'tbd', '']:
                    profile['director'] = director_name
                    print(f"  ✓ Added director '{director_name}' to {title}")
                else:
                    print(f"  ⚠️  Skipped invalid director '{director_name}' for {title}")
